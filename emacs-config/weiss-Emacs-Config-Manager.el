...
(defvar EmacsConfigManager-path (or (ignore-errors (executable-find "EmacsConfigManager"))
    ...
    (if (listp package)
        (let ((name (car package))
          ...
          (when (and (not (plist-get plist :disabled))
            ...
            (weiss-require-config-by-class (symbol-name name) log-file)
            ...
            (when (not (plist-get plist :skip-install)) 
              (cond
               ...
               ((plist-member plist :github)
                (setq install-command `(quelpa '(,name
                                                 :fetcher github-ssh
                                                 :repo ,(weiss-process-git-link (plist-get plist :github)))))                
                )
               ((plist-member plist :file)
                (setq install-command `(quelpa '(,name :fetcher file
                                                       :path ,(plist-get plist :file))))
                )
               ((plist-member plist :quelpa)
                ;; (message "quelpa: %s" (plist-get plist :quelpa))
                (setq install-command `(quelpa ',(plist-get plist :quelpa)))
                )
               ((plist-member plist :elpa)
                ;; (message "quelpa: %s" (plist-get plist :quelpa))
                (setq install-command `(package-install ',(plist-get plist :elpa)))
                )
               ;; ((plist-member plist :name)
               ;;  (message ": %s" (plist-get plist :quelpa))
               ;;  ;; (quelpa (plist-get plist :quelpa))
               ;;  )
               (t (setq install-command `(quelpa ',name)))
               )
              (when install-command
                (if log-file
                    (weiss-insert-to-log-file log-file (format "%s" install-command))
      ...
      (if log-file
          (progn
            (weiss-insert-to-log-file log-file (format "(quelpa '%s)" package))
